...
def run_all_checks(edit: "Edit", *, before_files: list[str] | None = None,
    ...
    payload; Python syntax runs only for complete files because replacement
    fragments are not valid standalone modules. `before_files`/`after_files` are optional
    ...
    # files; parsing a replacement fragment would create a false critical hit.
    if code is not None:
